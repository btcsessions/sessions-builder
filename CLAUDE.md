...
Gist sync now serves as an **encrypted off-machine backup**. It was built for multi-machine sync and would resume that role if a second machine returns; restoration is now explicit, not automatic at startup. Important:

- **Encryption**: all backed-up JSON files, including OAuth credentials, are encrypted with a user-set sync password before pushing to the Gist. This prevents GitHub secret scanning from detecting and revoking API keys (confirmed: both GitHub tokens and Anthropic keys were revoked this way). The password is set once per machine in Settings and stored locally only.
...
- **Fallback**: Without a sync password, settings are sanitized and OAuth credentials are never uploaded. New encrypted pushes remove plaintext filenames from the current Gist revision; older Gist history is not erased. Legacy backups can still be restored manually. Every restore validates the full input and keeps a local recovery snapshot.
...
The active Mac installation uses Python 3.14.7 in its venv; older system installations may use Python 3.9. Keep code 3.9-compatible (use `from __future__ import annotations` for `X | Y` type hints).

## Hermes/Umbrel Integration (client implemented; backend separate)
...
- **vidIQ integration** (`vidiq_client.py`): a stdlib MCP client (JSON-RPC over streamable HTTP, SSE-aware, cached session) that calls vidIQ's MCP server directly with a user-supplied connection key — Settings fields `vidiq_mcp_key` (in sync SECRET_KEYS), `vidiq_mcp_url` (blank = default), `vidiq_channel_id` (improves score accuracy). Both plan paths (Generate + Import Notes) use it: keyword research injects real volume/competition metrics into the prompt and merges top keywords into tags; after generation, titles are vidIQ-scored, topped up with vidIQ's own scored suggestions, deduped, and reordered best-first (`vidiq_scores` on the plan; colored vidIQ chips in the workspace next to the AI score badges). `/api/score-titles` attaches `vidiq` per title; `/api/vidiq/test` validates the key (0-credit call). Every vidIQ call is best-effort — failures log and generation proceeds without it. Tool calls cost vidIQ credits (~5 each, roughly 30–50 per plan).
...
- **Mobile access**: set `PLANNER_HOST=0.0.0.0` (and optionally `PLANNER_PORT`) before launch to reach the app from a phone on the same network at `http://<machine-ip>:5000`. Default stays `127.0.0.1`. Network clients must authenticate using `PLANNER_ACCESS_TOKEN` (set on the host); a sign-in form is provided. Use a trusted private network or HTTPS for network access. Optional DNS hostnames must be listed in `PLANNER_ALLOWED_HOSTS`.
...
## Safety and update compatibility

- Update App stays on the current branch, fetches and validates a candidate before fast-forwarding, and exec-restarts the same Python process. It refuses dirty tracked files and missing dependencies rather than partially changing the environment. This release adds no dependencies. Keep `/planner` returning 200 without redirects for old Dock launchers.
- On the first launch after this update, `updater.patch_installed_launcher()` replaces the known port-killing shell snippet in the existing project-local bundle and retains the old script. No Swift rebuild is needed for the standard port 5000 installation. Custom ports need a newly built launcher.
- Durable state remains compatible JSON in `data/`. `storage.py` provides owner-only atomic writes, previous-version `.bak` files, a shared lock, and journaled restore snapshots under `data/recovery/`. Do not write JSON directly. Browser `plannerState` retains its existing localStorage key and shape.
- Debug mode is off. A per-installation cookie signing secret is stored locally in `data/.session-secret`; no existing API keys or OAuth refresh tokens are rotated by an update. An OAuth authorization already in progress during the first update must be restarted because the cookie signing secret changes.
- `PLANNER_DATA_DIR` supports isolated fixtures. `PLANNER_SKIP_BACKGROUND=1` disables startup network jobs for tests. Run `python -m unittest discover -s tests -v` in the installed dependency environment. Tests use synthetic data and a disposable local Git remote.
- Trend intelligence returns last-known timestamped observations immediately while stale feeds refresh in a background thread.
